...
function scorematrix3(roundinfo::RoundInfo, feasiblepanels::Vector{AdjudicatorPanel})
    ...
    Γα = @spawnat 4 languagerepresentationmatrix(feasiblepanels, roundinfo)
    Φα = @spawnat 5 genderrepresentationmatrix(feasiblepanels, roundinfo)
    Ht = @spawnat 6 teamadjhistorymatrix(feasiblepanels, roundinfo)
    Ha = @spawnat 7 matrixfromvector(adjadjhistoryvector, feasiblepanels, roundinfo)
    Ct = @spawnat 8 teamadjconflictsmatrix(feasiblepanels, roundinfo)
    Ca = @spawnat 9 matrixfromvector(adjadjconflictsvector, feasiblepanels, roundinfo)
    Σ  = componentweights.language     * fetch(Γα)
    Σ += componentweights.gender       * fetch(Φα)
    Σ += componentweights.quality      * fetch(Π)
    Σ += componentweights.adjconflict  * fetch(Ca)
